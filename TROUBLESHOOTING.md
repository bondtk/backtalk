...
- **It crashes every time you ask it to look at a picture**: `logs/backtalk.log` shows `Fatal error in message reader: ... JSON message exceeded maximum buffer size`. The `claude_agent_sdk` transport (`_internal/transport/subprocess_cli.py`) reads the CLI subprocess's JSON stdout line-by-line into a buffer capped at 1MB by default (`_DEFAULT_MAX_BUFFER_SIZE`), and `brain.py`'s `_opts()` wasn't overriding it. A full-res photo's tool-result content, base64-encoded, sails past 1MB easily (a 3.7MB JPEG became a ~5MB JSON line) and kills the reader — and with it the whole session. Small screenshots can dodge this for a long time before a real camera photo finally trips it, which is why it can look brand-new. Fixed by setting `max_buffer_size=10 * 1024 * 1024` on the `ClaudeAgentOptions(...)` call in `_opts()`. If it recurs with even bigger images (RAW photos, multi-photo messages), raise that number further.
